...
export const handleSocket = (socket: Socket) => {
  ...
  socket.on("user:newEmployee", (newEmployee: User) =>
    user.newEmployee(socket, newEmployee)
  );
  ...
  socket.on("user:update", (updateUser: Partial<UserFull>, userId: number) =>
    user.update(socket, updateUser, userId)
  );
  ...
  // TALHAO OPS
  socket.on("talhao:create", (newTalhao: Talhao, isAdmin: boolean) =>
    talhao.newTalhao(socket, newTalhao, isAdmin)
  );
  ...
  socket.on("report:approve", (reportId: number) =>
  ...
  socket.on("notification:viewed", (id: number, user_id: number) =>
