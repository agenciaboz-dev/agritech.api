...
const approveCall = async (socket: Socket, data: ApproveCall) => {
    ...
    try {
        ...
        new Notification({ action: "approve", target_id: call.call.id, target_key: "call", users: [...employees, call.call.producer.user] })
    } catch (error) {
        console.log(error)
...
const closeCall = async (socket: Socket, data: Call) => {
    ...
    try {
        ...
        const employees = call.call.kit.employees.map((item) => item.user)
        new Notification({ action: "close", target_id: call.call.id, target_key: "call", users: [...employees, call.call.producer.user] })
    } catch (error) {
        console.error(error)
...
const cancelCall = async (socket: Socket, data: Call) => {
    ...
    try {
        ...
        const employees = call.call.kit.employees.map((item) => item.user)
        new Notification({ action: "cancel", target_id: call.call.id, target_key: "call", users: [...employees, call.call.producer.user] })
    } catch (error) {
        console.log(error)
