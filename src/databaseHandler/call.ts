...
const adminCreate = async (data: AdminCall) => {
    try {
        ...
        // Update the producer
        const findTalhao = await prisma.talhao.findUnique({ where: { id: data.talhaoId } })
        const findTillage = await prisma.tillage.findUnique({ where: { id: findTalhao?.tillageId } })
...
const list = async () => {
    ...
    const updatedCalls = await Promise.all(
        calls.map(async (call) => {
            const findTillage = await prisma.tillage.findUnique({ where: { id: call.talhao?.tillageId || 0 } })
